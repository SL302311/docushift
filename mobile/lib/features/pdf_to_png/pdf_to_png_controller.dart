...
import 'pdf_to_png_platform_interface.dart';
import '../pdf_page_range.dart';
import '../raster_resolution.dart';

/// DocuShift PDF 转 PNG — 控制器（第 4 期）。
///
/// 通过 [ChangeNotifier] 驱动 UI 更新。
/// 接受可注入的 [PdfToPngPlatform]，便于单元测试。
///
/// 两步选择流：选 PDF（原生验证 + 元数据）→ 选输出目录 → 转换。
/// 显示名/页数/大小来自平台，控制器不根据 URI 猜测。
class PdfToPngController extends ChangeNotifier {
  final PdfToPngPlatform _platform;

  PdfToPngController({PdfToPngPlatform? platform})
      : _platform = platform ?? MethodChannelPdfToPngPlatform();

  PdfToPngState _state = const PdfToPngState();
  PdfToPngState get state => _state;
  ...
  Future<void> _convert(String pdfUri, String directoryUri) async {
    ...
    try {
      final result = await _platform.convertPdfToPng(
        pdfUri,
        directoryUri,
        range.startPage,
        range.endPage,
  ...
  Future<void> retry() async {
    if (_state.status != PdfToPngStatus.failure) return;
