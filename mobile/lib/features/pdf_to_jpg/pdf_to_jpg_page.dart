...
import 'pdf_to_jpg_controller.dart';
import 'pdf_to_jpg_state.dart';
import '../raster_resolution.dart';

/// DocuShift PDF 转 JPG 页面（第 5 期）。
...
class _PdfToJpgPageState extends State<PdfToJpgPage> {
  final _controller = PdfToJpgController();
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              if (state.range != null &&
                  (state.status == PdfToJpgStatus.selected ||
                      state.status == PdfToJpgStatus.failure))
  ...
  Widget _buildPdfInfo(PdfToJpgState state) {
    final pdf = state.pdf!;
    return Card(
      child: ListTile(
  ...
  Widget _buildRangeSelector(PdfToJpgState state) {
    final pdf = state.pdf!;
  ...
  Widget _buildResolutionSelector(PdfToJpgState state) {
  ...
  Widget _buildAction(PdfToJpgState state) {
    switch (state.status) {
      case PdfToJpgStatus.idle:
      ...
      case PdfToJpgStatus.choosingFolder:
        return const Column(
          children: [
      ...
      case PdfToJpgStatus.converting:
        return const Column(
          children: [
            CircularProgressIndicator(),
            SizedBox(height: 12),
  ...
  String _statusText(PdfToJpgState state) {
    switch (state.status) {
      case PdfToJpgStatus.idle:
