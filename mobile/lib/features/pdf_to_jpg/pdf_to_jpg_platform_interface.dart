...
/// DocuShift PDF 转 JPG — 平台接口抽象（第 5 期）。
///
/// 将 MethodChannel 调用抽象为可注入接口，便于控制器测试。
abstract class PdfToJpgPlatform {
  ...
  /// 将 PDF 指定连续页范围渲染为 JPG 写入输出目录下新建子文件夹。
  ...
  Future<Map<String, dynamic>?> convertPdfToJpg(
    String pdfUri,
    String directoryUri,
    int startPage,
    int endPage, {
...
class MethodChannelPdfToJpgPlatform implements PdfToJpgPlatform {
  final MethodChannel _channel;

  MethodChannelPdfToJpgPlatform()
...
/// 假实现（用于单元测试）。
class FakePdfToJpgPlatform implements PdfToJpgPlatform {
  /// 下一个 pickPdf 调用的行为。
  ...
  /// 下一个 convertPdfToJpg 调用的行为（可断言参数透传）。
