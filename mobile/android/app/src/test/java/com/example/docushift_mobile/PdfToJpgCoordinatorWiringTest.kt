...
/**
 * PdfToJpgCoordinator 无设备接线测试（第 5 期）：
 * 真实执行 single-flight（compareAndSet）+ CompletionGuard + onDestroy 竞争逻辑，
 * 以及两步选择流（pickPdf 验证 → pickOutputDirectory → convertPdfToJpg）。
 *
 ...
 */
class PdfToJpgCoordinatorWiringTest {
    ...
    private fun coordinator(): PdfToJpgCoordinator {
        val c = PdfToJpgCoordinator()   // activity = null（测试路径）
    ...
    fun pickPdf_cancel_returnsNull() {
        val c = coordinator()
        c.pdfPicker = PdfToJpgCoordinator.PdfPicker { null }
    ...
    fun pickPdf_valid_returnsNativeMetadata() {
        val c = coordinator()
        c.pdfPicker = PdfToJpgCoordinator.PdfPicker { pdfUri }
        val r = FakeMethodResult()
        c.pickPdf(r)
    ...
    fun pickOutputDirectory_cancel_returnsNull_noFilesCreated() {
        ...
        c.convertExecutor = PdfToJpgCoordinator.ConvertExecutor { _, _ ->
            converted.set(true)
            "x" to 0L
        }
        c.treePicker = PdfToJpgCoordinator.TreePicker { null }
        val r = FakeMethodResult()
    ...
    fun pickOutputDirectory_success_returnsUri_andTakesPermission() {
        ...
        c.treePicker = PdfToJpgCoordinator.TreePicker { treeUri }
        val r = FakeMethodResult()
        c.pickOutputDirectory(r)
    ...
    fun convert_validationBeforeExecutor_failFastWithoutStartingWork() {
        ...
        val r = FakeMethodResult()
        c.convertPdfToJpg("content://in.pdf", "content://tree/out", r)
    ...
    fun convert_invalidRange_startGtEnd_invalidPageRange_noExecutor() {
        ...
        val r = FakeMethodResult()
        c.convertPdfToJpg("content://in.pdf", "content://tree/out", r, 4, 2)
    ...
    fun convert_invalidRange_endGtTotal_invalidPageRange() {
        val c = coordinator() // total = 3
        val r = FakeMethodResult()
        c.convertPdfToJpg("content://in.pdf", "content://tree/out", r, 1, 99)
        assertEquals("INVALID_PAGE_RANGE", r.errorCode.get())
    }
    ...
    fun convert_withRange_passesExactRange_andReturnsActualPageCount() {
        val c = coordinator() // total = 3
        val captured = mutableListOf<PdfToJpgConverter.ConvertParams>()
        c.convertExecutor = PdfToJpgCoordinator.ConvertExecutor { params, _ ->
            captured.add(params)
            "content://tree/out/report_JPG_x" to 123L
        }
        val r = FakeMethodResult()
        c.convertPdfToJpg("content://in.pdf", "content://tree/out", r, 2, 3)
    ...
    fun convert_success_returnsDirectoryPageCountSize() {
        val c = coordinator()
        c.convertExecutor = PdfToJpgCoordinator.ConvertExecutor { params, sink ->
            // 断言不可变参数来自验证结果
            ...
            "content://tree/out/report_JPG_x" to 999L
        }
        val r = FakeMethodResult()
        c.convertPdfToJpg("content://in.pdf", "content://tree/out", r)
        assertTrue(r.await(3000))
        @Suppress("UNCHECKED_CAST")
        val map = r.successValue.get() as Map<String, Any?>
        assertEquals("content://tree/out/report_JPG_x", map["directoryUri"])
        assertEquals(3, map["pageCount"])
    ...
    fun convert_singleFlight_secondCallBusy() {
        ...
        c.convertPdfToJpg("content://in.pdf", "content://tree/out", r1)
        assertTrue("executor 应已进入后台", started.await(3, TimeUnit.SECONDS))

        c.convertPdfToJpg("content://in.pdf", "content://tree/out", r2)
    ...
    fun convert_failure_cleansCreatedInReverseOrder_keepsOriginalError() {
        ...
        val u2 = Mockito.mock(Uri::class.java)  // 001.jpg
        val u3 = Mockito.mock(Uri::class.java)  // 002.jpg
        val deleted = mutableListOf<Uri>()
        c.outputDeleter = { uri -> deleted.add(uri); true }
        c.convertExecutor = PdfToJpgCoordinator.ConvertExecutor { _, sink ->
    ...
    fun pickPdf_busyWhilePending() {
        ...
        c.pdfPicker = PdfToJpgCoordinator.PdfPicker {
    ...
    fun pickOutputDirectory_busyWhilePending() {
        val c = coordinator()
        val second = FakeMethodResult()
        c.treePicker = PdfToJpgCoordinator.TreePicker {
