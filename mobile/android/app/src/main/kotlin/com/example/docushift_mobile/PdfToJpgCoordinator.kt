...
/**
 * DocuShift PDF → JPG — Android 端操作协调器（第 5 期）。
 *
 * 与 [PdfToPngCoordinator] 同构，复用同一套并发与生命周期契约，但委托
 * [PdfToJpgConverter] 完成逐页渲染为 JPG。
 *
 * 三方法契约（镜像 [PdfToPngCoordinator] 模式）：
 ...
 * - [convertPdfToJpg]：进入后台转换前再次验证（防止选择后文件变化），后台线程执行
 *   [PdfToJpgConverter.convert]；成功返回 `{directoryUri, pageCount, size}`。
 *
 * 并发与生命周期：
 ...
 * 默认走真实实现。
 */
open class PdfToJpgCoordinator(private val activity: FlutterFragmentActivity? = null) {
    ...
    /** 转换执行器，默认调用真实 [PdfToJpgConverter.convert]；createdSink 由本协调器持有。 */
    internal fun interface ConvertExecutor {
        fun convert(params: PdfToJpgConverter.ConvertParams, createdSink: MutableList<Uri>): Pair<String, Long>
    }
    ...
    private class ConvertRequest(
        ...
        val params: PdfToJpgConverter.ConvertParams,
    ...
    open fun convertPdfToJpg(
        ...
        val request = ConvertRequest(
            result,
            PdfToJpgConverter.ConvertParams(
                pdfUri = pdf,
                outputTreeUri = tree,
