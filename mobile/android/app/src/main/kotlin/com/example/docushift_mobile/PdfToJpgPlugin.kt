...
/**
 * DocuShift PDF → JPG — Flutter ↔ Android 平台通道（第 5 期，独立频道）。
 *
 * 三方法：`pickPdf` / `pickOutputDirectory` / `convertPdfToJpg`。
 * 委托 [PdfToJpgCoordinator] 处理实际 I/O 操作；通道名与 PDF→PNG 不同，
 * 二者互不干扰，PDF→PNG 的 MethodChannel 兼容契约保持不变。
 *
 * 结构性门禁（缺失参数 / 空字符串 → INVALID_ARGS）在本层快速失败，
 * 深层校验（页数 / 大小 / 可打开性 / BUSY）交给 Coordinator/Validator。
 */
class PdfToJpgPlugin internal constructor(
    private val activity: Activity
) : MethodChannel.MethodCallHandler {

    companion object {
        private const val CHANNEL = "com.example.docushift_mobile/pdf_to_jpg"
        private const val METHOD_PICK_PDF = "pickPdf"
        private const val METHOD_PICK_DIR = "pickOutputDirectory"
        private const val METHOD_CONVERT = "convertPdfToJpg"
        ...
        fun registerWith(flutterEngine: FlutterEngine, activity: Activity) {
            MethodChannel(
                ...
                CHANNEL
            ).setMethodCallHandler(PdfToJpgPlugin(activity))
        }
    }

    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        val coordinator = (activity as? MainActivity)?.pdfToJpgCoordinator
        if (coordinator == null) {
        ...
        when (call.method) {
            ...
            METHOD_CONVERT -> {
                ...
                coordinator.convertPdfToJpg(pdfUri, dirUri, result, startPage, endPage, resolution)
            }
